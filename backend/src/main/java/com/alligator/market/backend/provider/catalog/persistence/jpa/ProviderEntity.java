 ...
 * <ul>
 *     <li>Таблица {@code market_data_provider} является статичным справочником, предназначенным
 *     исключительно для отображения информации о провайдерах.</li>
 *     <li>Процесс обновления данных выполняется доменным процессом {@link ProviderSynchronizer}
 *     напрямую в БД через SQL (стратегия {@code DELETE -> INSERT}). Это выносит управление жизненным циклом записей
 *     за пределы механизмов отслеживания изменений JPA.</li>
 *     <li>Аннотации и инкапсуляция:
 *         <ul>
 *             <li>{@link Immutable} и отсутствие сеттеров отражают read-only статус сущности для Hibernate.</li>
