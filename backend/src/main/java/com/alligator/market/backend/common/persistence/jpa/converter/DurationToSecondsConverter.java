...
/**
 * Конвертер {@link Duration} ⟷ число секунд ({@link Long}) для хранения в БД.
 *
 * <p><b>Семантика преобразования</b>
 * <ul>
 *   <li>В БД пишутся <em>целые</em> секунды из {@link Duration#getSeconds()} — дробная часть
 *       (наносекунды) безвозвратно отбрасывается.</li>
 *   <li>Для отрицательных значений действует поведение {@code getSeconds()} (округление <em>вниз</em>):
 *       {@code PT1.9S → 1}, а {@code PT-1.9S → -2}.</li>
 *   <li>{@code null} поля сущности → {@code null} в колонке и наоборот.</li>
 * </ul>
 *
 * <p><b>Требования к схеме БД</b>
 * <ul>
 ...
 * </ul>
 *
 * <p><b>Когда этот конвертер подходит</b>
 * <ul>
 *   <li>Нужно хранить длительности с точностью до секунды.</li>
 *   <li>Допустима потеря субсекундной части.</li>
 * </ul>
 *
 * <p><b>Когда выбрать другой подход</b>
 * <ul>
