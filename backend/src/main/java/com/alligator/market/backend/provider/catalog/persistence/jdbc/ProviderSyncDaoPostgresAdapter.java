 ...
 * <p>DAO без состояния; JdbcTemplate потокобезопасен. Методы допустимо вызывать из разных потоков,
 * но по смыслу синхронизация выполняется один раз при старте и целиком в @Transactional-сервисе.</p>
 * <p>В кластере запускайте синк только на «лидере» (ShedLock/PG advisory lock), чтобы избежать гонок.</p>
 * <p>UPSERT идемпотентен, удаление безопасно при одинаковом входном наборе кодов.</p>
 *
 * <p><b>Ограничения:</b></p>
 * <ul>
 *   <li>БД — PostgreSQL;</li>
